...
if __name__ == "__main__":
    ...
    annotation_data = np.load(Path(args.annotation_directory) / args.brain_ID / str(args.brain_ID+"_annotation_results.npz"), allow_pickle=True)
    annotations = annotation_data["annotations"]
    name_map = dict(annotation_data["name_map"])

    csv_path = glob.glob(str(args.sample_data + args.brain_ID + '*.csv'))[0]
    sample_data = pd.read_csv(csv_path, index_col="sample_id")
    ...
    for idx, row in sample_data.iterrows():
        if not np.isnan(row['segmentation_row']):
