...
GG changes:
    - made selecting midlines figure smaller so fits on screen - otherwise midline selection is inaccurate
    - added 'Brain_ID' as the basefolder and this is used in the filename of the segmented images
    - added '//' to the filepath which allows the tifs to be imported??
...
def segment_coronal_slice_hemisphere(img, midline, sample_ids=None, sample_coordinates=None, show=False):
    ...
    midpoint_mask = np.full_like(equalized,0)
    
    mp = midpoint.astype(int)
    midpoint_mask[mp[1]-10:mp[1]+10 , mp[0]-10:mp[0]+10] = 100
